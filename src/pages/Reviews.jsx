      ...
      {[1, 2, 3, 4, 5].map(n => (
        <button key={n} type="button"
          ...
          <Star className="w-8 h-8 transition-colors"
            fill={(hover || value) >= n ? '#A67C68' : 'none'}
            style={{ color: (hover || value) >= n ? '#A67C68' : 'rgba(166,124,104,0.25)' }} />
        </button>
      ))}
    </div>
  );
  ...
  return (
    <div className="min-h-screen font-tajawal" style={{ background: '#F4F1EA' }} dir={dir}>
      ...
      <div className="relative py-20 px-6 text-center overflow-hidden"
        style={{ background: 'linear-gradient(160deg, #F4F1EA 0%, #EFE9DD 60%, #F4F1EA 100%)' }}>
        <div className="absolute top-4 inset-x-0 flex justify-center z-20">
          <LanguageSwitcher dark={false} />
        </div>
        <div className="absolute inset-0 opacity-[0.04]"
          style={{ backgroundImage: 'linear-gradient(#A67C68 1px, transparent 1px), linear-gradient(90deg, #A67C68 1px, transparent 1px)', backgroundSize: '60px 60px' }} />
        <div className="relative z-10 max-w-2xl mx-auto">
          <p className="text-xs tracking-[0.4em] font-bold mb-3 uppercase" style={{ color: '#A67C68' }}>{t('reviews.eyebrow')}</p>
          <h1 className="text-5xl md:text-6xl font-black mb-4" style={{ color: '#3E322D' }}>{t('reviews.title')}</h1>
          <div className="w-24 h-0.5 mx-auto mb-6" style={{ background: 'linear-gradient(90deg, transparent, #A67C68, transparent)' }} />
          <p className="text-base" style={{ color: 'rgba(62,50,45,0.5)' }}>{t('reviews.subtitle')}</p>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        ...
        <div className="max-w-xl mx-auto mb-20">
          <div className="rounded-2xl p-8" style={{ background: 'rgba(62,50,45,0.045)', border: '1px solid rgba(166,124,104,0.15)' }}>
            {submitted ? (
              <div className="text-center py-8">
                <CheckCircle className="w-16 h-16 mx-auto mb-4" style={{ color: '#A67C68' }} />
                <h3 className="text-xl font-black mb-2" style={{ color: '#3E322D' }}>{t('reviews.thanksTitle')}</h3>
                <p className="text-sm mb-6" style={{ color: 'rgba(62,50,45,0.5)' }}>{t('reviews.thanksDesc')}</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                <h2 className="text-xl font-black mb-6" style={{ color: '#3E322D' }}>{t('reviews.formTitle')}</h2>
                <div>
                  <label className="text-sm font-bold mb-2 block" style={{ color: 'rgba(62,50,45,0.7)' }}>{t('reviews.nameLabel')}</label>
                ...
                <div>
                  <label className="text-sm font-bold mb-2 block" style={{ color: 'rgba(62,50,45,0.7)' }}>{t('reviews.ratingLabel')}</label>
                  <StarRating value={form.rating} onChange={v => setForm(f => ({ ...f, rating: v }))} />
                </div>
                <div>
                  <label className="text-sm font-bold mb-2 block" style={{ color: 'rgba(62,50,45,0.7)' }}>{t('reviews.textLabel')}</label>
                ...
                <div>
                  <label className="text-sm font-bold mb-2 block" style={{ color: 'rgba(62,50,45,0.7)' }}>{t('reviews.serviceLabel')}</label>
                  <Input value={form.service} onChange={e => setForm(f => ({ ...f, service: e.target.value }))}
                    placeholder={t('reviews.servicePh')} maxLength={100}
                    className="bg-transparent border-white/10 text-white placeholder:text-white/20 focus:border-yellow-500/50" />
                </div>
                <div>
                  <label className="text-sm font-bold mb-2 block" style={{ color: 'rgba(62,50,45,0.7)' }}>{t('reviews.orderLabel')}</label>
        ...
        {reviews.length > 0 && (
          <div>
            <h2 className="text-2xl font-black text-center mb-10" style={{ color: '#3E322D' }}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {reviews.map((r, i) => (
                <motion.div key={r.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  style={{ background: 'rgba(62,50,45,0.045)', border: '1px solid rgba(166,124,104,0.12)' }}>
                  <div className="flex gap-0.5">
                    {[...Array(5)].map((_, s) => (
                      <Star key={s} className="w-4 h-4"
                        fill={s < r.rating ? '#A67C68' : 'none'}
                        style={{ color: s < r.rating ? '#A67C68' : 'rgba(166,124,104,0.2)' }} />
                    ))}
                  </div>
                  <p className="text-sm leading-relaxed flex-1" style={{ color: 'rgba(62,50,45,0.65)' }}>"{r.text}"</p>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-sm" style={{ color: '#3E322D' }}>{r.customer_name}</p>
                      {r.service && <p className="text-xs" style={{ color: 'rgba(62,50,45,0.3)' }}>{r.service}</p>}
                    </div>
                  </div>
                </motion.div>
              ))}
        ...
        <div className="text-center mt-16">
          <Link to="/booking">
            <button className="px-8 py-3 rounded-full font-bold text-sm transition-all hover:scale-105"
              style={{ background: 'rgba(166,124,104,0.1)', border: '1px solid rgba(166,124,104,0.25)', color: '#A67C68' }}>
              {t('reviews.backHome')}
            </button>
          </Link>
        </div>
