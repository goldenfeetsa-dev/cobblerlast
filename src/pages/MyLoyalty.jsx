  ...
  return (
    <div className="space-y-5">
        ...
        <div className="space-y-2">
          {result.notifications.filter((n) => !n.is_read).map((n) => (
            <div key={n.id} className="rounded-xl p-3 flex items-start gap-2 text-sm"
              style={{ background: 'rgba(166,124,104,0.08)', border: '1px solid rgba(166,124,104,0.2)', color: '#3E322D' }}>
              <Bell className="w-4 h-4 shrink-0 mt-0.5" style={{ color: '#A67C68' }} />
      ...
      <div className="rounded-2xl p-6 text-center"
        style={{ background: 'linear-gradient(135deg, rgba(166,124,104,0.12), rgba(166,124,104,0.03))', border: '1px solid rgba(166,124,104,0.25)' }}>
        <p className="text-xs font-mono mb-1" style={{ color: 'rgba(166,124,104,0.6)' }}>{member.member_number}</p>
        <h2 className="text-2xl font-black mb-1" style={{ color: '#3E322D' }}>{member.full_name}</h2>
        <span className="inline-block px-3 py-1 rounded-full text-xs font-bold mb-4"
          style={{ background: 'rgba(166,124,104,0.15)', color: '#A67C68' }}>
          {LEVEL_LABELS_AR[member.membership_level] || member.membership_level}
        </span>
        <div className="text-5xl font-black mb-1" style={{ color: '#A67C68' }}>{member.points}</div>
        <p className="text-xs mb-5" style={{ color: 'rgba(62,50,45,0.4)' }}>نقطة</p>
      ...
      <div>
        <p className="text-xs text-center mb-2" style={{ color: 'rgba(62,50,45,0.35)' }}>اختر نوع محفظتك</p>
      ...
      <div className="rounded-2xl p-5" style={{ background: 'rgba(62,50,45,0.035)', border: '1px solid rgba(166,124,104,0.1)' }}>
        <h3 className="flex items-center gap-2 text-sm font-bold mb-3" style={{ color: '#3E322D' }}>
          <History className="w-4 h-4" style={{ color: '#A67C68' }} /> آخر العمليات
        </h3>
        {(!result.history || result.history.length === 0) ? (
          <p className="text-xs" style={{ color: 'rgba(62,50,45,0.4)' }}>لا توجد عمليات بعد.</p>
        ) : (
          <div className="space-y-2">
            {result.history.slice(0, 10).map((tx) => (
              <div key={tx.id} className="flex items-center justify-between text-xs py-2"
                style={{ borderTop: '1px solid rgba(166,124,104,0.08)' }}>
                <span style={{ color: 'rgba(62,50,45,0.6)' }}>{tx.reason || 'بدون سبب'}</span>
    ...
    <div className="min-h-screen py-8 px-4 font-tajawal" dir="rtl"
      ...
      <div className="max-w-lg mx-auto relative z-10">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-black" style={{ color: '#3E322D' }}>بطاقة الولاء الخاصة بك</h1>
          <p className="text-sm mt-2" style={{ color: 'rgba(62,50,45,0.4)' }}>
            {member ? 'بطاقتك جاهزة — أضفها لمحفظة جوالك' : 'أنشئ عضويتك بثوانٍ، أو ابحث عن بطاقة موجودة'}
          </p>
        </div>

        {!member && (
          <div className="flex gap-2 mb-6 p-1 rounded-xl" style={{ background: 'rgba(62,50,45,0.045)' }}>
            <button onClick={() => setTab('join')}
              className="flex-1 py-2.5 rounded-lg text-sm font-bold transition-all"
              style={tab === 'join'
                ? { background: 'linear-gradient(135deg, #A67C68, #C9A08D)', color: '#000' }
                : { color: 'rgba(62,50,45,0.5)' }}>
              عضوية جديدة
            </button>
        ...
        {!member && tab === 'join' && (
          <div className="rounded-2xl p-5 mb-6 space-y-3"
            ...
            <div className="relative">
              <User className="w-4 h-4 absolute right-3 top-1/2 -translate-y-1/2" style={{ color: 'rgba(166,124,104,0.5)' }} />
            ...
            <div className="relative">
              <PhoneIcon className="w-4 h-4 absolute right-3 top-1/2 -translate-y-1/2" style={{ color: 'rgba(166,124,104,0.5)' }} />
        ...
        {searched && !loading && !member && tab === 'lookup' && (
          <div className="text-center py-14 rounded-2xl" style={{ background: 'rgba(62,50,45,0.035)', border: '1px solid rgba(166,124,104,0.1)' }}>
            <p style={{ color: 'rgba(62,50,45,0.4)' }}>لم يتم العثور على بطاقة ولاء بهذه البيانات.</p>
          </div>
        )}
