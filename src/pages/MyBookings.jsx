    ...
    <div className="min-h-screen py-8 px-4 font-tajawal" dir={dir}
      ...
      <div className="max-w-lg mx-auto relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-black" style={{ color: '#F5EDD8' }}>{t('myBookings.title')}</h1>
          <p className="text-sm mt-2" style={{ color: 'rgba(245,237,216,0.4)' }}>{t('myBookings.subtitle')}</p>
        </div>
        ...
        {searched && !isLoading && bookings.length === 0 && (
          <div className="text-center py-14 rounded-2xl" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(201,168,76,0.1)' }}>
            ...
            <p style={{ color: 'rgba(245,237,216,0.4)' }}>{t('myBookings.noBookings')}</p>
            <Link to="/book">
              <button className="mt-5 px-7 py-3 rounded-full font-bold text-black transition-all hover:scale-105"
                style={{ background: 'linear-gradient(135deg, #C9A84C, #e8c96a)' }}>{t('myBookings.bookNow')}</button>
            </Link>
          </div>
        )}
            ...
            return (
              <div key={booking.id} className="rounded-2xl p-5 transition-all"
                ...
                <div className="mt-4 pt-4 flex items-center justify-between" style={{ borderTop: '1px solid rgba(201,168,76,0.1)' }}>
                  <span className="text-xs" style={{ color: 'rgba(245,237,216,0.3)' }}>{t('myBookings.total')}</span>
                  <span className="font-black text-lg" style={{ color: '#C9A84C' }}>{booking.total_price} SAR</span>
                </div>
              </div>
            );
          })}
